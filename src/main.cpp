...
uint256 hashGenesisBlock("0x691edcf0b16b79b0a599dd31e451cf566fbb3b24c556bd7f87b686acc62a9de4");//
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 16); // Greenpay: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Greenpay Signed Message:\n";
...
{
    // Greenpay: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // nSubsidy >>= (nHeight / 840000); // Greenpay: 840k blocks in ~4 years
...
//    nSubsidy>>=count;
    ...
    else if(nHeight<=365*3*2880)
        nSubsidy=8.4* COIN;//10000000/(30*2880)
    else
        nSubsidy=0.0001*COIN;
    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 30*60; // Greenpay: 30min
static const int64 nTargetSpacing = 1 * 30; // Greenpay:30s
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Greenpay: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Greenpay: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Greenpay: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Created By zaobi.org .QQ:260682605 ,欢迎咨询!2016-3-17";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0x90971ca71284ef604117a80995f4d8705dc4d26f39f00407597cfa166fd2e41b"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Greenpay: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("greenpayMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("greenpayMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static greenpayMiner(CWallet *pwallet)
{
    printf("greenpayMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("greenpay-miner");
    ...
    try { while(true) {
        ...
        printf("Running greenpayMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("greenpayMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&greenpayMiner, pwallet));
}
